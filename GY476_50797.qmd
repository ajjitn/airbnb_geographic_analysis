...
# hex_grid_airbnb_variables %>% 
#   filter(!is.na(avg_log_hex_price)) %>% 
#   ggplot() +
#   geom_sf(aes(fill = avg_log_hex_price), 
#           color = "white") +
...
# Generate grid of hex id and price_ntile combos, and append actual hex geos
hex_price_ntile_combos = expand_grid(hex_id = unique(airbnb_data_log_corrected$hex_id),
            price_ntile = unique(airbnb_data_log_corrected$price_ntile),
            manually_added = TRUE) %>% 
  left_join(hex_grid_acs_variables %>% select(hex_id),
            by = "hex_id")


airbnb_data_by_hex_and_price_quantile_with_hex_geos = airbnb_data_log_corrected %>% 
  st_drop_geometry() %>% 
  mutate(log_price = log(price)) %>% 
  # Each row = hex-price_quantile combo in airbnb data
  group_by(hex_id, price_ntile) %>% 
  summarize(num_listings = n(),
            avg_price = mean(price),
            avg_log_price = mean(log_price)) %>% 
  # Join with full grid of hex <> price_ntile combinations
  full_join(hex_price_ntile_combos,
            by = c("hex_id", "price_ntile")) %>% 
  st_as_sf() %>% 
  st_centroid()
...
ggplot() +
  ...
  scale_fill_distiller(na.value = "grey40",
                       ...
                       direction = 1,
                       labels = scales::dollar_format()) +
  geom_sf(data = airbnb_data_by_hex_and_price_quantile_with_hex_geos,
          aes(color = num_listings),
          alpha = 0.5,
          stroke = FALSE) +
  scale_color_distiller(
                       #na.value = "grey40",
  ...
  geom_sf(data = airbnb_data_log_corrected, 
          ...
          alpha = 0.6,
          size = 0.3) +
  scale_color_fermenter(na.value = "grey40",
                       palette = "Oranges",
                       direction = 1) +
...
Generally we see that dark orange points tend to be around, and close to dark blue neighborhoods, but not necessarily in them. In other words, expensive Airbnbs tend to be near expensive neighborhoods which makes sense as some of these expensive neighborhoods may be tourist destinations and housing may be scarce in those neighborhoods. The ZCTA level correlation between average listing price and median rent for the 167 ZCTAs in our study area is `r zcta_level_correlation_price_rent` and therefore backs up our reading of the map as a weakly positive relationship between median ZCTA rent and Airbnb price.  
