...
# Define study area as airbnb locations bounding box + buffer
study_area_box = airbnb_data_projected %>%
  # Since CRS is in meters, this is 500 meter boundingbox
  # st_buffer(dist = 500) %>% 
  st_bbox() 

# Limit ZCTA data to this bounding box
acs_data_cropped = acs_data_raw %>% 
  mutate(zcta_area = st_area(.)) %>% 
  st_crop(study_area_box)

zcta_in_bbox = acs_data_cropped %>% st_drop_geometry() %>% pull(zcta) %>% unique()

mapview(acs_data_raw %>% filter(zcta %in% zcta_in_bbox)) | mapview(acs_data_cropped, col.regions = "steelblue")

mapview(acs_zctas_with_airbnbs) |  mapview(acs_data_cropped, col.regions = "steelblue")
...
problematic_listings_matched_zctas = airbnb_data_joined %>% 
  filter(is.na(zcta)) %>%
  st_is_within_distance(acs_data_raw, 400) %>%
  # Above returns either index if match is found or an empty set if no matches.
  # Take first index (ie closest matching zcta index) and extract actual zcta
  # value from acs_data_raw
  map(~acs_data_raw %>% 
        slice(.x) %>% 
        pull(zcta)) %>%
  ...
  unlist() %>% 
  as_tibble_col(column_name = "zcta_corrected") %>% 
...
airbnb_data_corrected = airbnb_data_joined %>% 
  left_join(problematic_listings_matched_zctas, 
            by = "id") %>% 
  mutate(zcta_final = coalesce(zcta, zcta_corrected)) 
  # removes the 20 unmatched listings


airbnb_data_corrected %>% 
  filter(is.na(zcta_final)) %>% 
  mapview() + mapview(acs_data_cropped, col.regions = "steelblue")


 

list_zctas_in_airbnb_data = airbnb_data_joined %>% 
  select(zcta) %>% 
  st_drop_geometry() %>% 
  pull(zcta) %>% 
  unique()
...
acs_zctas_with_airbnbs = acs_data_raw %>% 
  filter(zcta %in% zctas_in_airbnb_data) 

x = airbnb_data_joined %>% 
...
mapview(acs_data_raw %>% st_crop(airbnb_data_projected))
x %>% 
  filter(is.na(zcta)) %>% 
  mapview() | mapview(acs_data_raw %>% st_crop(airbnb_data_projected))
