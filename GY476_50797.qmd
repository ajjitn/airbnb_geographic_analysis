...
ggplot() +
  ...
  scale_fill_distiller(na.value = "grey40",
                       ...
                       direction = 1,
                       labels = scales::dollar_format()) +
  geom_sf(data = airbnb_data_corrected, 
          color = "#EF8A17",
          stroke = FALSE,
          alpha = 0.02) +
...
Map 3 shows all the airbnb listing locations (orange points) overlaid on top of a chorpoleth map of Median rent. I have set increased transparency of the Median Rent choropleth map and increased transparency of the Airbnb point listings so that we can easily see how the listings line up with Median rent and reduce visual clutter. Generally we see that listings are generally concentrated around (but not necessarily in) neighborhoods with high median rent particularly in San Francisco.
