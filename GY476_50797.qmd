...
This paper will look at how airbnb listings are distributed across the Bay Area, how they are correlated with housing costs, 

The American Community Survey data we will be using is a classic example of "old" spatial data, which are traditional, detailed datasets that are often specifically designed and collected for use in social science research. They are very detailed, accurate,a and have margins of error, as they are often assembled by teams of statisticians in government agencies (in this case the Census Bureau). The problems with these "old" spatial data are that they are very costly to produce, and therefore usually only available at low frequencies. And because of privacy concerns, they are often only available at coarse geographic resolutions.

This is in contrast to the Airbnb data, which is a "new" form of spatial data. These newer forms of spatial data were created as a part of the geo-data revolution which is producing more geographic data than ever before. geo-data are often collected automatically and assembled for business purposes and purely by accident are also available for researchers. The benefits of these data are that they are available at very frequent intervals (our Airbnb data for example were all scrpaed within the last month), have very granular geographies, and cover many populations not traditionally covered by old spatial data. 

The disadvantages of these new spatial datasets are that they are subject to many biases, often do not contain measures of uncertainty, and may violate privacy rights by allowins individuals to be reidentified. In the case of Airbnb data for example, we only have pricing data on the small sliver of the rental market that chooses to upload their listings to Airbnb, and is not at all representative of the larger housing, or even tourist/rental market. Furthermore, while Airbnb does some validation, it is not comprehensive and it is entirely possible that this data include fake listings which would skew the data considerably. 

# Data Collection 


We first programmatically download our data directly from URLs to make the data import process as reproducible as possible. 
...
# Download and read in Airbnb data, we got these URLs from looking at developer console in HTML pages
...
- OSM Bike Rental Location: Using the overpass API and `library(osmdata)` in R, we download OSM Amenity data on bike rental stations in the Bay Area. These are user reported point locations of where bike stations are located in Open Street Map and include both private and municipal bike rental locations.

## Data Cleaning

For the Airbnb data, we convert the `price` column from a character vector into a numeric vector by removing commas, and `$` signs from the column. This will allow us to treat this column as numeric when performing analyses later. After some exploratory analysis, we also decide to drop the top 0.5% most expensive Airbnb listings from this analysis as they are very high outliers. This corresponds to dropping 67 listings with prices greater than or equal to $2500 per night. For context, the vast majority of listings have prices between \$50 and \$500 a night. And upon visual inspection we see that most of these dropped listings don't have any reviews and may not be real, justifying our decision. 
...
For the San Francisco County boundaries, we remove some small islands which are included in the county boundary becuase the maps we will be making with these data only include bike stations within mainland San Francisco. And while removing the small islands is a small factual omission, it will make the resulting San Francisco level maps much more readable and allow users to focus in on the patterns of importance within mainland San Francisco.

```{r cleaning_sfo_county_data}
#| cache: true
#| output: false
...
# Trim SFO boundary to just mainalnd SF, and remove small islands

# SF county includes some small islands like Treasure island, and an island in 
# the Pacific. To make our maps more readable, we remove these islands from the
# extent of the county
sfo_county_boundary = sfo_county %>% 
  # Convert myultipolygon into polygon data, which splits up one row sf dataframe
  # into multi row sf dataframe with each distinct polygon getting its own row
  st_cast("POLYGON") %>% 
  mutate(area = st_area(.)) %>% 
  # Keep largest area polygon, which shouuld just be mainland SF county polygon
  top_n(1, area)
```

For the OSM bike data, we  notice that there are many duplicate looking bikeshare stations. After doing a distance based join, we count 178 stations which are within 2 meters of each other and which are probably user reported duplicates. To avoid double counting these stations, we only keep the first ocurrence of these duplicate rows, thereby dropping 98  duplicate bikeshare stations from the data. We also filter the data to just stations that are a part of the Bay Wheels or Ford GoBike networks - as captured by the `network` and `brand` columns in the OSM data. We want to focus our analysis on the Bay Wheels netwok as they have a large system of docked bikeshare stations throughout San Francisco, and Ford GoBike was the old name of the system [@said_2019] which still appears in OSM data. Finally, we do not perform any data cleaning on the ACS data as it has aready been thoroughly cleaned and validated by the Census Bureau.
...
# ---- Clean up OSM bike data --------

# There are 178 stations that are within 2 meters of another station. We remove 
# these 89 probable duplicates
dupe_list = st_is_within_distance(bike_rental_points, bike_rental_points, 2)
...
#     # take the centriod of the 2 points for most accuracy, and to collapse to one point for future analysis
# }
# 
...
## Projections 
I will use `EPSG:7131`or the NAD83 State Plane projection for San Franscisco for all our datasets as this was one of two projected coordinate reference systems specifically created by the San Francisco City and County GIS department. This is a projected 2D conformal coordinate system that:
...
bike_rental_points_in_network = bike_rental_points_in_network %>% 
  st_transform("EPSG:7131")

sfo_county_boundary = sfo_county_boundary %>% 
  st_transform("EPSG:7131")
...
We want to focus our bike rental analysis to just San Francisco County, so we use a spatial join to filter to bikeshare stations in San Francisco. 

```{r}
#| cache: true 

# ---- Crop bike rental points to SF ----
...
We also create a heaxgonal honeycomb grid, which will aid us when creating heatmaps, and allow us to visualize the relative density and prices of airbnb listings and bike stations easily (See Map 3 and Map 4 for examples of these hex grids in action). 

```{r}
#| cache: true

# ---- Create hex honeycomb grid for data ----
...
1) I decide to bin our continuous data into intervals because the ranges of our count and price variables are relatively large and we want to avoid having too many colors as we might have with a continuous color ramp. And using bins makes the resulting colors easier to read and interpret for a user. I specifically use manually created equal interval bins in order to give a reader an honest view of variation in the data.
...
3) I choose to include the surrounding ZCTA boundaries as a basemap layer and do not actually map their values (which in this case would be 0 becuase there are no Airbmbs in these surrounding ZCTAs). I intentionally do not list these light gray ZCTAs in the legend as I do not want the reader to focus on these neighborhoods. They exist only to frame the outline of the Bay Area and perhaps serve as helpful geographic context for readers familiar with the Bay Area.

4) I use the readymade Green-Blue color palette from [ColorBrewer](https://colorbrewer2.org/#type=sequential&scheme=GnBu&n=6), based on Cynthia Brewer's research [-@harrower2003colorbrewer] on gradients that are colorblind friendly, and suitable for LCD computer screens.

5) Finally we also provide interactive maps using `library(mapview)` just for exploratory purposes, which can be be found in the `interactive` tabs.
...
Most Airbnbs tend to be between \$150 - \$300 a night. The highest priced Airbnbs tend to be along the Pacific Ocean coast, and along the coastline of San Francisco. This could be because these Airbnbs have great views, or access to beaches/other tourist friendly amenities. Conversely, the lowest priced Airbnbs tend to be in Oakland. 
...
# mapview(hex_grid_acs_variables,
#         zcol = "pct_bach_higher_e",
#         col.regions=brewer.pal(9, "Blues"),
#         layer.name = )
# 
# mapview(hex_grid_airbnb_variables %>% filter(!is.na(avg_hex_price)),
#         zcol = "number_of_listings",
...
Map 3 shows the average log price of Airbnb listings (orange points) overlaid on top of a chorpoleth map of Median rent. Darker orange points mean more expensive listings, and darker blue neighborhoods are higher median rent neighborhoods. I have made a couple of decisions in presenting the data that are justified below:

1) I have aggregated the 13,000+ Airbnb points into hexagon grids and presented the average log price of all listings within the hexagon. This is because displaying all 13,000 points on the map was way too cluttered and we needed some way to aggregate points. I considered aggregating the listings to ZCTAs, but ZCTAs have very different sizes and I wanted aggregations of roughly equal areas. 

2) I have used an interactive map because the ability to toggle layers on and off is very helpful when comparing the scales of two variables as we are doing here. In this case, if you hover over the layer button (right below the zoom button) and toggle the `Median Rent` layer on and off repeatedly. you can visually compare exactly where the low and high priced airbnb neighborhoods overlap the high and low median rent neighborhoods. 

2) I have set increased transparency of the Median Rent layer so that we can visually send that layer into the background as we have already seen this distribution before in other maps. This also allows us to more easily see how listing prices line up with Median rent. 

3) I have turned the map title into a pseudo-legend by making use of colors. This helps prime the reader for the two different variables that they will be seeing on the map. I also include two  continuous color ramp legends to represent the scales of the different colors.

4) There are 4 listings which have a list price of \$0 a night. I manually increment these values to be \$1 a night so that when we take  `log(price)`, the result will be well defined and not be -infinity. 

Generally we see that neighborhoods with high listing prices (dark orange) tend to clustered near the coast, and not necesarily near high rent (dark blue) neighborhoods. There are some clusters of high listing price neighborhoods close to high rent neighborhoods, particularly in San Mateo near the Silicon Valley belt. But the highest listing neighborhoods are not generally in the highest rent neighborhoods. The ZCTA level correlation between average listing price and median rent for the 167 ZCTAs in our study area is `r zcta_level_correlation_price_rent` and therefore backs up our reading of a pretty weakly positive relationship between median ZCTA rent and Airbnb price.  
...
```
 l
This maps shows that most bike rental locations are located in San Francisco, along with seemingly a second network in Oakland, and just a handful in San Mateo. Digging deeper into the OSM data, we see that there are a total of `r nrow(bike_rental_points_sfo)` bike rental stations in our study area. And while most of these don't have explicit names, there are many marked as belonging to Bay Wheels, which is the main bikeshare station serving San Francisco, San Jose and the East Bay.

### Creating buffers around bike share stations
...
Next, we create 200 meter buffers around the bike share stations in our study area, and then perform a spatial join to see which Airbnbs fall within these buffer areas. Overall,  `r count_airbnbs_close_to_bikes` unique Airbnbs (or `r ((count_airbnbs_close_to_bikes/ count_total_airbnbs_in_sfo) %>% round(3))*100`% of Airbnbs) are within the 200 meter buffer of bike stations. For me, this would help decide where to choose an Airbnb if I was going to San Francisco as I personally love biking. But I would have doubts about the effectiveness of this strategy given that Open Street Map data can often be incomplete or out of date. So it is likely that this is not a full or accurate list of bikeshare friendly Airbnbs that meet our criteria.
