...
The American Community Survey data we will be making use of in this paper is a classic example of "old" spatial data, which  are traditional, detailed datasets that are often specifically designed and collected for use in social science research. They are very detailed, accurate and 


The problems with these "old" spatial datasets is that they are very costly to produce, and therfore usually only available at low frequencies. And because of privacy concerns, they are often only available at coarser geographic resolutions. 

This is in contrast to the Airbnb data, which is a "new" form of spatial data
...
We also create a heaxgonal honeycomb grid for our point level listing data, which will aid us when creating heatmaps, and allow us to visualize the relative density and prices of airbnb listings easily (See Map 3 and Map 4 for examples of these hex grids in action). 
...
# ---- Calculate hexagon level variables using intersections ----
...
```{r}
#| cache: true
hex_grid_airbnb_variables %>% filter(!is.na(avg_hex_price))


hex_grid_airbnb_variables = hex_grid_airbnb_variables %>% 
  mutate(log_avg_hex_price = log(avg_hex_price))
...
This maps shows that most bike rental locations are located in San Francisco, along with seemingly a second network in Oakland, and just a handful in San Mateo. Digging deeper into the OSM data, we see that there are a total of `r nrow(bike_rental_points_joined %>% filter(in_study_area))` bike rental stations in our study area. And while most of these don't have explicit names, there are `r bike_rental_points_joined %>% filter(in_study_area) %>% count(brand) %>% filter(brand == "Bay Wheels") %>% pull(n)`marked as belonging to Bay Wheels, which is the main bikeshare station serving San Francisco, San Jose and the East Bay.
...
bike_rental_points_buffered = bike_rental_points_joined %>% 
  filter(in_study_area) %>% 
  st_buffer(200)
